...
class TestScenePatchCli(unittest.TestCase):
    def setUp(self):
        self.tmp_dir = Path("tests/temp_test_cli")
    ...
    def test_cmd_patch_build_and_splice(self):
        ...
        # This has word count 100 which is within 80-120 range (±20% of 100)
        # Note: word count of clean draft is 100 words.
        clean_text = "This is a replacement segment of prose. " * 10
        (folder / "replacement.md").write_text(
            f"```markdown\n"
            f"This is the original draft.\n"
            f"{clean_text}\n"
            f"Goodbye world.\n"
            f"```",
            encoding="utf-8"
        ...
        self.assertIn("This is a replacement segment", draft_p.read_text(encoding="utf-8"))
